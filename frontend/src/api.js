...
export const api = {
  // Network
  getStats: () => fetch(`${API_BASE}/network/stats`, { cache: 'no-store' }).then(r => r.json()),
  getTransformers: () => fetch(`${API_BASE}/network/transformers`, { cache: 'no-store' }).then(r => r.json()),
  getPoles: (dtId) => fetch(`${API_BASE}/network/poles${dtId ? `?dt_id=${dtId}` : ''}`, { cache: 'no-store' }).then(r => r.json()),
  getTopology: (dtId) => fetch(`${API_BASE}/network/topology/${dtId}`, { cache: 'no-store' }).then(r => r.json()),
  getTopologySummary: () => fetch(`${API_BASE}/network/topology-summary`, { cache: 'no-store' }).then(r => r.json()),
  
  // Pole state
  getDarkPoles: () => fetch(`${API_BASE}/poles/dark`, { cache: 'no-store' }).then(r => r.json()),
  getPoleState: (poleId) => fetch(`${API_BASE}/poles/state/${poleId}`, { cache: 'no-store' }).then(r => r.json()),
  
  // Tickets
  getActiveTickets: () => fetch(`${API_BASE}/tickets?active=true`, { cache: 'no-store' }).then(r => r.json()),
  getTicket: (id) => fetch(`${API_BASE}/tickets/${id}`, { cache: 'no-store' }).then(r => r.json()),
  ...
  // Scheduled outages
  getActiveOutages: () => fetch(`${API_BASE}/scheduled-outages/active`, { cache: 'no-store' }).then(r => r.json()),
  ...
  getActiveFaults: () => fetch(`${API_BASE}/simulator/active-faults`, { cache: 'no-store' }).then(r => r.json()),
